...
echo "    /oktoklaw [file]"
echo ""
echo "  Spine state: telemetry loaded from runtime memory when available; unknown values remain unknown."
echo "  f(x) = x is a design invariant; it is not a guarantee that runtime state is correct."
echo ""

# Optional env setup prompt
echo "  Set environment variables for full integration:"
echo "    export GITHUB_ACCESS_TOKEN=<runtime-secret>"
echo "    export SLACK_WEBHOOK_URL=<runtime-secret>"
echo "    export STRIPE_SECRET_KEY=<runtime-secret>"
echo "    export DATABASE_URL=<runtime-secret-or-local-config>"
echo ""
